...
const VARIANT_NAMES := ["Пиратский разведчик", "Пиратский истребитель",
						...
						"ЭЛИТНЫЙ ФЛАГМАН ПРОТИВНИКА"]

# ── Weapon hardpoints ─────────────────────────────────────────────────────────
# Local coords: x=right, y=fwd (both as fraction of ship sz). Up to 6 slots.
const HARDPOINT_LOCAL: Array = [
	Vector2( 0.0,   0.82),   # 0: bow
	Vector2(-0.95,  0.05),   # 1: port wing
	Vector2( 0.95,  0.05),   # 2: starboard wing
	Vector2( 0.0,  -0.60),   # 3: stern
	Vector2(-0.55,  0.55),   # 4: port forward
	Vector2( 0.55,  0.55),   # 5: starboard forward
]
const HARDPOINT_HIT_RADIUS := 20.0  # px — projectile pos within this range damages the weapon
const HARDPOINT_DOT_R      := 5.5   # draw radius for hardpoint indicator
...
var _show_results:   bool  = false  # показывать оверлей итогов
...
# ── Weapon damage & ammo ─────────────────────────────────────────────────────
var weapon_damaged:    Array = []   # bool per weapon slot — true = disabled
var player_hit_radius: float = 28.0
...
func _calc_enemy_fleet() -> Array:
	...
	# Danger 5: главный враг — Элитный флагман если main_variant не задан особо
	if danger == 5 and main_variant <= 3:
		# Заменяем главного врага на элитного флагмана
		fleet[0] = _make_enemy_data(4, -1, danger, true)

	# Additional enemies
	for i in base_count - 1:
		var v: int
		if danger == 5:
			v = 3 if i == 0 else randi() % 3  # дредноут + свита
		elif danger >= 4:
			v = randi() % 3
		elif danger >= 3:
			v = randi() % 2
...
func _make_enemy_data(variant: int, preset_hull: int, danger: int, is_main: bool) -> Dictionary:
	...
	match variant:
		...
		3:  # Dreadnought (danger 5 only)
			...
			move_speed = 28.0
		4:  # Элитный флагман (только danger 5 — требуется флагманский корабль)
			base_hull = 2200
			attack_interval = 0.55  # очень быстрый огонь
			move_speed = 38.0
		_:
			base_hull = 80
			attack_interval = 2.5
...
func _init_combat() -> void:
	...
	for wname in GameManager.equipped_weapons:
		for w in GameData.WEAPONS:
			if w["name"] == wname:
				...
				# Инициализируем патроны: из сохранёнки если есть, иначе полный боезапас
				if wd.has("ammo"):
					wd["ammo_left"] = GameManager.weapon_ammo_state.get(wname, wd["ammo"])
				player_weapons.append(wd)
				break
	if player_weapons.is_empty():
		player_weapons.append({"name": "Импульсное орудие", "damage": 25, "type": "pulse", "ammo_left": 200})
	...
	# Инициализируем состояние повреждений орудий
	weapon_damaged.clear()
	for i in player_weapons.size():
		weapon_damaged.append(GameManager.damaged_weapons.has(i))

	# Радиус попадания зависит от размера корабля
	player_hit_radius = _get_ship_sz() * 0.75
...
func _handle_auto_fire(delta: float) -> void:
	...
	# Check weapon hardpoint damage
	if selected_weapon_idx < weapon_damaged.size() and weapon_damaged[selected_weapon_idx]:
		_status_lbl.text = "⚠  Орудие «%s» повреждено! Ремонт в доке." % w["name"]
		auto_fire_timer = 0.6
		return
	...
	# Check ammo (all weapons have ammo_left now)
	if w.has("ammo_left") and w["ammo_left"] <= 0:
		_status_lbl.text = "⚠  Боезапас «%s» пуст! Пополните в космопорту." % w["name"]
		auto_fire_timer = 0.5
		return
...
func _enemy_fire(e: Dictionary) -> void:
	...
	# Elite flagship — variant 4
	if variant == 4:
		_enemy_fire_flagship(e, fwd)
		return
...
func _enemy_fire_flagship(e: Dictionary, fwd: Vector2) -> void:
	var pattern := randi() % 5
	match pattern:
		0:  # Orbital railgun — single massive slug
			enemy_projectiles.append(_make_proj(e["pos"] + fwd * 44,
				e["angle"] + randf_range(-0.01, 0.01), 320, "kinetic", false))
			_status_lbl.text = "💥💥  ФЛАГМАН — ОРБИТАЛЬНЫЙ РЕЛЬСОТРОН!"
		1:  # Total missile saturation
			for sp: float in [-0.30, -0.16, -0.06, 0.06, 0.16, 0.30]:
				enemy_projectiles.append(_make_proj(e["pos"] + fwd * 44,
					e["angle"] + sp, 90, "missile", true))
			_status_lbl.text = "🚀🚀🚀  ФЛАГМАН — ПОЛНЫЙ РАКЕТНЫЙ ЗАЛП!"
		2:  # Plasma + kinetic barrage
			for sp: float in [-0.20, -0.07, 0.07, 0.20]:
				enemy_projectiles.append(_make_proj(e["pos"] + fwd * 44,
					e["angle"] + sp, 75, "plasma", false))
			enemy_projectiles.append(_make_proj(e["pos"] + fwd * 44,
				e["angle"], 200, "kinetic", false))
			_status_lbl.text = "☄💥  ФЛАГМАН — ПЛАЗМА И КИНЕТИЧЕСКИЙ УДАР!"
		3:  # EMP grid — wide spread
			for _i in 8:
				enemy_projectiles.append(_make_proj(e["pos"] + fwd * 44,
					e["angle"] + randf_range(-0.55, 0.55), 55, "emp", false))
			_status_lbl.text = "⚡⚡  ФЛАГМАН — ЭМИ-ШТОРМ! ЩИТЫ БЕСПОЛЕЗНЫ!"
		4:  # All-weapons alpha strike
			enemy_projectiles.append(_make_proj(e["pos"] + fwd * 44,
				e["angle"], 250, "kinetic", false))
			for sp: float in [-0.14, 0.14]:
				enemy_projectiles.append(_make_proj(e["pos"] + fwd * 44,
					e["angle"] + sp, 80, "plasma", false))
			for sp: float in [-0.28, -0.10, 0.10, 0.28]:
				enemy_projectiles.append(_make_proj(e["pos"] + fwd * 44,
					e["angle"] + sp, 60, "missile", true))
			_status_lbl.text = "☠  ФЛАГМАН — ПОЛНЫЙ АЛЬФА-УДАР! ВСЕМ УКРЫТЬСЯ!"
...
# ── Weapon hardpoint hit detection ───────────────────────────────────────────

func _check_hardpoint_hit(impact: Vector2, proj_type: String) -> void:
	var n: int = player_weapons.size()
	if n == 0: return
	var sz: float = _get_ship_sz()
	var fwd  := Vector2(sin(player_angle), -cos(player_angle))
	var rgt  := fwd.rotated(PI / 2.0)
	for si in n:
		if si >= HARDPOINT_LOCAL.size(): break
		if weapon_damaged.size() > si and weapon_damaged[si]: continue
		var loc: Vector2 = HARDPOINT_LOCAL[si]
		var hp_world: Vector2 = player_pos + rgt * loc.x * sz + fwd * loc.y * sz
		if impact.distance_to(hp_world) < HARDPOINT_HIT_RADIUS:
			weapon_damaged[si] = true
			var wname: String = player_weapons[si].get("name", "орудие")
			_status_lbl.text = "⚠  ОРУДИЙНЫЙ ОТСЕК ПРОБИТ: «%s»!" % wname
			AudioManager.play_sfx("hurt")
			return  # не более одного повреждения за попадание

func _get_ship_sz() -> float:
	match GameManager.current_ship.get("ship_type", "Исследовательский"):
		"Боевой":            return 36.0
		"Штурмовой":         return 36.0
		"Грузовой":          return 44.0
		"Ресурсодобывающий": return 34.0
		"Флагманский":       return 60.0
		"Дипломатический":   return 40.0
		"Пиратский":         return 38.0
		_:                   return 32.0
...
func _update_projectiles(delta: float) -> void:
	...
	for i in range(enemy_projectiles.size() - 1, -1, -1):
		...
		var fwd := Vector2(sin(p["angle"]), -cos(p["angle"]))
		p["pos"] += fwd * (MISSILE_SPEED if p["is_missile"] else LASER_SPEED) * delta
		if p["pos"].distance_to(player_pos) < player_hit_radius:
			_player_take_damage(p["damage"], p.get("type", "energy"), p["pos"] as Vector2)
...
func _player_take_damage(amount: int, proj_type: String = "energy", impact: Vector2 = Vector2.ZERO) -> void:
	...
	# Проверяем попадание по орудийным отсекам
	if impact != Vector2.ZERO:
		_check_hardpoint_hit(impact, proj_type)
...
func _save_hull() -> void:
	...
	# Сохраняем повреждённые орудия
	GameManager.damaged_weapons.clear()
	for i in weapon_damaged.size():
		if weapon_damaged[i]:
			GameManager.damaged_weapons.append(i)
	# Сохраняем остатки боезапаса
	GameManager.weapon_ammo_state.clear()
	for w in player_weapons:
		if w.has("ammo_left"):
			GameManager.weapon_ammo_state[w["name"]] = int(w["ammo_left"])
...
func _draw() -> void:
	...
	if _show_results:
		_draw_end_overlay(vp)

func _draw_end_overlay(vp: Vector2) -> void:
	var victory := not _pending_defeat
	...
	# Рамка
	var box_w: float = 480.0; var box_h: float = 260.0
	var box := Rect2(Vector2(cx - box_w * 0.5, cy - 30), Vector2(box_w, box_h))
	draw_rect(box, Color(0.05, 0.07, 0.16, 0.92))
	draw_rect(box, Color(0.22, 0.40, 0.75, 0.55), false, 2.0)
	...
	if victory:
		title = "★  ПОБЕДА!  ★"
		tcol  = Color(0.22, 1.0, 0.45)
	else:
		title = "✕  ПОРАЖЕНИЕ  ✕"
		tcol  = Color(1.0, 0.22, 0.22)

	for gi in range(5, 0, -1):
		draw_circle(Vector2(cx, cy + 22), float(gi) * 22.0,
			Color(tcol.r, tcol.g, tcol.b, 0.03 + float(5 - gi) * 0.005))
	draw_string(font, Vector2(cx - 200, cy + 38),
		title, HORIZONTAL_ALIGNMENT_CENTER, 400, 42, tcol)
	draw_line(Vector2(cx - 200, cy + 48), Vector2(cx + 200, cy + 48),
		Color(tcol.r, tcol.g, tcol.b, 0.35), 1.0)

	# Итоги
	var sy: float = cy + 74.0
	var rows: Array = [
		["⚔  Нанесено урона",      str(_damage_dealt),    Color(1.0, 0.65, 0.25)],
		["🛡  Поглощено урона",     str(_damage_absorbed), Color(0.35, 0.65, 1.0)],
		["💀  Уничтожено кораблей", str(_ships_destroyed), Color(0.85, 0.30, 0.30)],
	]
	if victory:
		rows.append(["💰  Получено кредитов", "+%d" % _pending_earned, Color(0.25, 0.95, 0.45)])

	for row in rows:
		draw_string(font, Vector2(cx - 195, sy),
			row[0], HORIZONTAL_ALIGNMENT_LEFT, 250, 15, Color(0.70, 0.72, 0.80, 0.95))
		draw_string(font, Vector2(cx + 195, sy),
			row[1], HORIZONTAL_ALIGNMENT_RIGHT, 150, 17, row[2])
		sy += 28.0

	draw_string(font, Vector2(cx - 160, sy + 18),
		"— возврат к системе... —",
		HORIZONTAL_ALIGNMENT_CENTER, 320, 12, Color(0.38, 0.45, 0.58, 0.75))
...
func _draw_player_ship(pos: Vector2, angle: float) -> void:
	...
	if player_shields > 0 and player_max_shields > 0:
		...
		var sr := sz + 8.0
		draw_arc(p, sr,      0, TAU, 56, Color(0.22, 0.62, 1.0, 0.08 + sp * 0.28), 2.5)
		draw_arc(p, sr - 6,  0, TAU, 40, Color(0.40, 0.78, 1.0, (0.08 + sp * 0.28) * 0.40), 1.5)
	match ship_type:
		"Боевой":            _draw_ship_combat(p, angle,   Color(0.18, 0.50, 0.90))
		"Штурмовой":         _draw_ship_combat(p, angle,   Color(0.55, 0.18, 0.90))
		...
		"Дипломатический":   _draw_ship_scout(p, angle,    Color(0.55, 0.88, 1.0))
		"Пиратский":         _draw_ship_combat(p, angle,   Color(0.85, 0.22, 0.22))
		_:                   _draw_ship_scout(p, angle,    Color(0.22, 0.60, 0.90))
	# Weapon hardpoint indicators
	_draw_weapon_hardpoints(p, angle)
...
# ── Weapon hardpoint drawing ─────────────────────────────────────────────────

func _draw_weapon_hardpoints(pos: Vector2, angle: float) -> void:
	var sz: float  = _get_ship_sz()
	var fwd  := Vector2(sin(angle), -cos(angle))
	var rgt  := fwd.rotated(PI / 2.0)
	var n: int = player_weapons.size()
	for si in n:
		if si >= HARDPOINT_LOCAL.size(): break
		var loc: Vector2 = HARDPOINT_LOCAL[si]
		var hp: Vector2  = pos + rgt * loc.x * sz + fwd * loc.y * sz
		var is_dmg: bool = si < weapon_damaged.size() and weapon_damaged[si]
		if is_dmg:
			# Damaged: red pulsing indicator
			var pulse: float = 0.55 + sin(time_e * 18.0) * 0.35
			draw_circle(hp, HARDPOINT_DOT_R + 2.0, Color(0.9, 0.1, 0.1, pulse * 0.55))
			draw_circle(hp, HARDPOINT_DOT_R, Color(1.0, 0.18, 0.12, 0.95))
			draw_circle(hp, HARDPOINT_DOT_R - 2.0, Color(1.0, 0.55, 0.3, 0.80))
			draw_arc(hp, HARDPOINT_DOT_R + 3.0, 0, TAU, 10, Color(1.0, 0.3, 0.1, 0.50), 1.0)
		else:
			# OK: colored by weapon type
			var wtype: String = player_weapons[si].get("type", "energy")
			var wc := Color(0.22, 0.88, 0.55, 0.90)
			match wtype:
			...
			draw_circle(hp, HARDPOINT_DOT_R, wc)
			draw_circle(hp, HARDPOINT_DOT_R - 2.0, Color(1.0, 1.0, 1.0, 0.55))

# ── Player ship silhouettes ───────────────────────────────────────────────────

func _draw_ship_scout(pos: Vector2, angle: float, col: Color) -> void:
	var sz := 32.0; var fwd := Vector2(sin(angle), -cos(angle)); var right := fwd.rotated(PI/2.0)
...
func _draw_ship_combat(pos: Vector2, angle: float, col: Color) -> void:
	var sz := 36.0; var fwd := Vector2(sin(angle), -cos(angle)); var right := fwd.rotated(PI/2.0)
...
func _draw_ship_cargo(pos: Vector2, angle: float, col: Color) -> void:
	var sz := 44.0; var fwd := Vector2(sin(angle), -cos(angle)); var right := fwd.rotated(PI/2.0)
...
func _draw_ship_flagship(pos: Vector2, angle: float, col: Color) -> void:
	var sz := 60.0; var fwd := Vector2(sin(angle), -cos(angle)); var right := fwd.rotated(PI/2.0)
...
func _draw_hud(vp: Vector2, arena: float) -> void:
	...
	for i in player_weapons.size():
		...
		var is_dmg: bool  = i < weapon_damaged.size() and weapon_damaged[i]
		var ammo_txt: String = ""
		if is_dmg:
			ammo_txt = "  ⚠ПОВР."
			wcol = Color(1.0, 0.28, 0.18, 0.92)
		elif w.has("ammo_left"):
			var al: int = int(w["ammo_left"])
			if al <= 0:
				ammo_txt = "  [ПУСТО]"
